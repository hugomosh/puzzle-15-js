...
function createBoard() {
  ...
  var m = 4,
      n = 4;
  var getX = getXforMN(m, n),
      getY = getYforMN(m, n);
  var draw = svg_js_1.SVG().addTo("#puzzle-15").size(m * squareSize, n * squareSize);
  var board = {
    m: m,
    n: n,
    getX: getX,
...
function animateToNewState(newState) {
  var moves = {};
  var timeline = new svg_js_1.Timeline();
  timeline.pause();

  for (var index = 0; index < board.currentState.length; index++) {
    var currentTileId = board.currentState[index];
